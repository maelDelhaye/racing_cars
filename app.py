...
#initialisation de la liste pour le highscore 
liste_score = []
...
def menu_etat():
    ...
    if etat == 0 and pyxel.btnr(pyxel.KEY_RETURN):
        etat = 1
    elif etat != 4 and pyxel.btnr(pyxel.KEY_P):
        etat = 4
...
def maxi(listescore):
    """determination du highscore"""
    highscore = listescore[0]
    longueur = len(listescore)
    for i in range (longueur):
        if listescore[i] > highscore:
            highscore = listescore[i]
    return highscore
...
def draw():
    ...
    if etat == 0:
        ...
        pyxel.text(70,180, 'appuyez sur ENTER pour jouer', 0)
    
    # menu de règles
    elif etat == 1:
        pyxel.bltm(0, 0, 3, 512, 256, 256, 256)
        pyxel.text(105,40, 'REGLES DU JEU', 7)
        pyxel.text(21,90, "Eliminez le plus d'obstacles",0)
        pyxel.text(21,110, 'Utilisez les fleches de direction pour vous deplacer', 0)
        pyxel.text(21,130, 'Appuyez sur ESPACE pour tirer', 0)
        pyxel.text(21,150, 'Appuyez sur P pour arreter le jeu', 0)
        pyxel.text(21,170, 'Vous avez le choix entre 2 niveaux:', 0)
        pyxel.text(31,190, 'Pour le niveau facile, appuyez sur F', 0)
        pyxel.text(31,210, 'Pour le niveau difficile, appuyez sur D', 0)
    # mode facile
    elif etat==2 : 
    ...
    # affichage du menu de fin, game over
        ...
        liste_score.append(point)
        pyxel.bltm(0, 0, 1, 512, 256, 256, 256)
        pyxel.text(151,75, '!GAME OVER!', 8)
        pyxel.text(147,90, 'VOTRE SCORE: '+str(point), 0)
        pyxel.text(135,105, 'VOTRE HIGHSCORE: '+str(maxi(liste_score)), 0)
        pyxel.text(119,118, 'pour rejouer appuyer sur R', 0)
        pyxel.stop()
